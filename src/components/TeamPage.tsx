...
export function TeamPage() {
  return (
    <main className="hub-page" aria-label="Team contacts">
      <header className="hub-page-head">
        <p className="hub-eyebrow">Who to ping</p>
        <h2 className="hub-page-title">Team</h2>
        <p className="hub-page-lede">
          Contact roster for pad days, design reviews, and logistics. Edit names
          and emails in <code>hubData.ts</code>.
        </p>
      </header>
        ...
        <ul className="hub-contact-list">
          {CONTACTS.map((contact) => (
            <li key={contact.id} className="hub-contact">
              ...
              {contact.notes ? <p className="hub-contact-notes">{contact.notes}</p> : null}
            </li>
          ))}
        </ul>
      </section>
